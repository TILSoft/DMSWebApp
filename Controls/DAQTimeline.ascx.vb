...
#Region "properties"
    ...
    Private _OEE As Integer
    Private _TimeOfComp As DateTime
    Private _OEEview As Boolean
    Private _OEEMode As String
    ...
    Public Property TimeOfComp() As DateTime
        Get
            Return _TimeOfComp
    ...
    Public Property OEEView() As Boolean
        Get
            Return _OEEView
        ...
        End Set
    End Property
    Public Property OEEMode() As String
        Get
            Return _OEEMode
...
#End Region
    ...
    Sub CreateChart()
        ...
        ChartImage = New Bitmap(width + widthadjust, (lvlHeight * 2) + 40 + 40, PixelFormat.Format32bppRgb)
        ...
        Dim txtBrushG = New SolidBrush(Color.Green)
        ...
        g.DrawLine(linePen, 0, lvlHeight * 2 + 40, width, lvlHeight * 2 + 40)

        If OEEView Then
            g.DrawRectangle(linePen, ChartImage.Width - 1 - widthadjust, 0, ChartImage.Width - 1, ChartImage.Height - 1)
            If OEEMode = "Lot" Then
                'calculate batch count

                g.DrawString("Cartons: ", New Font("Veranda", 8), txtBrush, 10, lvlHeight * 2 + 42)
                g.DrawString(CurrentCount, New Font("Veranda", 20), txtBrushG, 55, lvlHeight * 2 + 41)
                g.DrawString("(" & Math.Round(CurrentCount / TargetCount * 100, 0) & "%)", New Font("Veranda", 10), txtBrushG, 5, lvlHeight * 2 + 55)
                g.DrawString("Target: ", New Font("Veranda", 8), txtBrush, 195, lvlHeight * 2 + 42)
                g.DrawString(TargetCount, New Font("Veranda", 20), txtBrushG, 235, lvlHeight * 2 + 41)
                g.DrawString("Batch OEE: ", New Font("Veranda", 8), txtBrush, 350, lvlHeight * 2 + 42)
                g.DrawString(OEE & "%", New Font("Veranda", 20), txtBrushG, 420, lvlHeight * 2 + 41)
                'calc estimated finish time
                g.DrawString("Est. completion: ", New Font("Veranda", 8), txtBrush, 510, lvlHeight * 2 + 42)
                g.DrawString(TimeOfComp, New Font("Veranda", 8), txtBrushG, 605, lvlHeight * 2 + 41)

            ElseIf OEEMode = "ChangeOver" Then


                Dim timeleft As Integer = DateDiff(DateInterval.Minute, Now(), TimeOfComp)
                If timeleft < 0 Then
                    txtBrushG.Color = Color.Red
                End If

                g.DrawString("Change Over Target Completion Time: ", New Font("Veranda", 8), txtBrush, 10, lvlHeight * 2 + 42)
                g.DrawString(TimeOfComp, New Font("Veranda", 20), txtBrushG, 230, lvlHeight * 2 + 41)
                g.DrawString("Time Left: ", New Font("Veranda", 8), txtBrush, 600, lvlHeight * 2 + 42)
                g.DrawString(timeleft, New Font("Veranda", 20), txtBrushG, 660, lvlHeight * 2 + 41)
        ...
        End If
            ...
            ImageMap1.ImageUrl = "~\Daqfrontend\charttemp\" & pngFileName & ".png"
    ...
    Sub addActBlocks()
        ...
        Do Until dtrows = daqActData.Rows.Count
            ...
            If _click_enabled Then
                ...
                rect1.NavigateUrl = "~/daqfrontend/ActivityDetails.aspx?activityID=" & navigateURL
                rect1.Target = "_blank"
        ...
        Do Until dtrows = daqData.Rows.Count
            ...
            If _click_enabled And drawHotSpot Then
                ...
                If downtimeid > 0 Then
                    ...
                    rect1.NavigateUrl = "~/daqfrontend/downtimedetails.aspx?downtimeID=" & downtimeid
                    rect1.Target = "_blank"
                    imgmap.HotSpots.Add(rect1)
                Else
