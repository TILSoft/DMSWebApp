...
Partial Class safehouse_testcontrol_Default3
    ...
    Sub DrawChart(ByVal ds As DataSet)
        ...
        For Each row As DataRow In ds.Tables(0).Rows
            ...
            ele.Name = row.Item("Year").ToString() & " - " & row.Item("Period").ToString()
            SCact.AddElements(ele)

            Dim avgEle As New Element

            avgEle.YValue = average
            avgEle.Name = row.Item("Year").ToString() & " - " & row.Item("Period").ToString()
