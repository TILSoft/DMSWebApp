...
Partial Class zAdministration_ManageShifts
    ...
    Protected Sub btnAddShift_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddShift.Click

        'check that shift doesnot conflict with another shift
        LblMessage.Text = ""
        Dim ShiftConflict As New Helpers

        If Not TreeView1.SelectedValue = Nothing Then



            If ShiftConflict.checkForShiftCross(CDate(txtStartTime.Text), CDate(txtEndTime.Text), TreeView1.SelectedValue) = 0 Then

                Dim shiftadap As New ShiftsTableAdapters.tblShiftsTableAdapter
                shiftadap.Insert(TreeView1.SelectedValue, txtStartTime.Text, txtEndTime.Text, txtteam.Text)
                GridView1.DataBind()
                LblMessage.Text = "Shift successfully added"

            Else

                LblMessage.Text = "This shift conflicts with one already entered"
            End If
        Else
            LblMessage.Text = "Please select a line from the list"
        End If

    End Sub

    Protected Sub lnkShiftWiz_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkShiftWiz.Click
        Response.Redirect("ShiftCopyWizard.aspx?LocID=" & TreeView1.SelectedValue)
    End Sub
